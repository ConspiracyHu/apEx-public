...
CWBApplication::CWBApplication() : CCoreWindowHandlerWin()
{
  ...
  RegisterUIFactoryCallback( _T( "fieldset" ), CWBFieldSet::Factory );
  RegisterUIFactoryCallback( _T( "label" ), CWBLabel::Factory );
  RegisterUIFactoryCallback( _T( "textbox" ), CWBTextBox::Factory );
  RegisterUIFactoryCallback( _T( "list" ), CWBList::Factory );
  RegisterUIFactoryCallback( _T( "dropdown" ), CWBDropDown::Factory );
  RegisterUIFactoryCallback( _T( "trackbar" ), CWBTrackBar::Factory );
}
...
LRESULT CWBApplication::WindowProc( UINT uMsg, WPARAM wParam, LPARAM lParam )
{
  ...
  case WM_KEYDOWN:
    UpdateControlKeyStates();
    //LOG_DBG("[app] Keydown: %d %d %d %d",wParam,Alt,Ctrl,Shift);
