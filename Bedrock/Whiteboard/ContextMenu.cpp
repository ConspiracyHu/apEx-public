...
CWBContextItem* CWBContextItem::GetItem( const CString& text )
{
  for ( int x = 0; x < Children.NumItems(); x++ )
    if ( Children[ x ]->Text == text )
      return Children[ x ];
  return nullptr;
}
...
CWBContextItem* CWBContextMenu::GetItem( const CString& Text )
{
  for ( TS32 x = 0; x < Items.NumItems(); x++ )
    if ( Items[ x ]->Text == Text )
      return Items[ x ];
  return nullptr;
}
