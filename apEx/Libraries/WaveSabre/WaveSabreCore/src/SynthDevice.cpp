...
namespace WaveSabreCore
{
		...
		: Device(numParams)
	{
		...
		voiceMode = VoiceMode::VoiceMode_Polyphonic;
		monoActive = false;
		noteCount = 0;
	...
	void SynthDevice::Run(double songPosition, float **inputs, float **outputs, int numSamples)
	{
		...
		while (numSamples)
		{
						...
						case EventType::EventType_NoteOn:
							{
								...
								switch (voiceMode)
								{
								case VoiceMode::VoiceMode_Polyphonic:
								default:
									...
									break;
								case VoiceMode::VoiceMode_MonoLegatoTrill:
			...
			for (int i = 0; i < maxEvents; i++)
			{
				if (events[i].Type != EventType::EventType_None) events[i].DeltaSamples -= samplesToNextEvent;
			}
	...
	void SynthDevice::NoteOn(int note, int velocity, int deltaSamples)
	{
		for (int i = 0; i < maxEvents; i++)
		{
			...
			{
				events[i].Type = EventType::EventType_NoteOn;
	...
	void SynthDevice::NoteOff(int note, int deltaSamples)
	{
		for (int i = 0; i < maxEvents; i++)
		{
			if (events[i].Type == EventType::EventType_None)
			{
				events[i].Type = EventType::EventType_NoteOff;
				events[i].DeltaSamples = deltaSamples;
				events[i].Note = note;
				break;
			}
	...
	{
		for (int i = 0; i < maxEvents; i++) events[i].Type = EventType::EventType_None;
	}
}
