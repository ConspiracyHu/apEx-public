...
namespace WaveSabreCore
{
	BiquadFilter::BiquadFilter()
	{
		...
		type = BiquadFilterType::BiquadFilterType_Lowpass;
	...
	float BiquadFilter::Next(float input)
	{
		if (recalculate)
		{
			...
			switch (type)
			{
			case BiquadFilterType::BiquadFilterType_Lowpass:
				a0 = 1.0f + alpha;
			...
			case BiquadFilterType::BiquadFilterType_Highpass:
				a0 = 1.0f + alpha;
				a1 = -2.0f * (float)Helpers::FastCos(w0);
				a2 = 1.0f - alpha;
				b0 = (1.0f + (float)Helpers::FastCos(w0)) / 2.0f;
			...
			case BiquadFilterType::BiquadFilterType_Peak:
				{
