	...
	bool SongRenderer::renderThreadWork(int renderThreadIndex)
	{
		...
		while (trackRenderStates[numTracks - 1] != TrackRenderState::TrackRenderState_Finished)
		{
			...
			for (int i = 0; i < numTracks; i++)
			{
				...
				{
					if (trackRenderStates[tracks[i]->Receives[j].SendingTrackIndex] != TrackRenderState::TrackRenderState_Finished)
					{
				...
				if ((TrackRenderState)InterlockedCompareExchange((unsigned int *)&trackRenderStates[i], (unsigned int)TrackRenderState::TrackRenderState_Rendering, (unsigned int)TrackRenderState::TrackRenderState_Idle) == TrackRenderState::TrackRenderState_Idle)
				{
