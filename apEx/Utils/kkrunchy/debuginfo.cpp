...
sChar *DebugInfo::WriteReport()
{
	...
	sSPrintF(Report, 512, "\nFunctions by Class/Namespace and packed size:\n");
	Report += sGetStringLen(Report);

	for (i = 1; i<Symbols.Count; i++)
		for (j = i; j>0; j--)
		{
			sInt f1 = Symbols[j].NameSpNum;
			sInt f2 = Symbols[j - 1].NameSpNum;

			if (f1 == -1 || f2 != -1 && sCmpStringI(NameSps[f1].Name.String, NameSps[f2].Name.String) < 0)
				sSwap(Symbols[j], Symbols[j - 1]);
		}
	...
	for (i = 0; i < Symbols.Count; i++)
	{
		if (Symbols[i].Class == DIC_CODE)
		{
			sSPrintF(Report, 512, "%5d.%02d/%8d: %-50s %s\n",
					 (sInt)Symbols[i].PackedSize, sInt(Symbols[i].PackedSize * 100) % 100,
					 Symbols[i].Size, Symbols[i].Name, NameSps[Symbols[i].NameSpNum].Name);
